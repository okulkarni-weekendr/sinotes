            ...
            if(text !== ''){
                if(newDepthMap.has(depth)){
                    ...
                    this.setState((prevState, array) => {
                        depthMap: prevState.depthMap.set(depth, array)
                    }, () => {
                        this.addToChildMap(depth, text)
                    });
                }else{
                    this.setState((prevState) => {
                        depthMap: prevState.depthMap.set(depth, [])
                    }, () => {
                        this.addToChildMap(depth, text)
                    });
                }
    ...
    addToChildMap(depth, text){
       ...
       console.log(depth);
        if(depthMap.has(depth-1)){
            ...
            this.setState((prevState, array) => {
                childMap:  prevState.childMap.set(key, array).set(text, [])
            });
        }else{
            this.setState((prevState) => {
                childMap: prevState.childMap.set(text, [])
            });
        }
    }
