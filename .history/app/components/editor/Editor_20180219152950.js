...
class Editor2 extends Component {
    ...
    handleKeyCommand(command){
        ...
        if (command === 'saveChart') {
            ...
            console.log('printEditorStateMap');
            return 'handled';
        }
        return 'not-handled';
    }
    ...
    printMapOfEditorState(){
        ...
        //     console.log(k.getText(),k.getDepth());
        // })
        let tempMap = new Map();
        editorState.getCurrentContent().getBlockMap().forEach(k => {
            if(k.getText().replace(/^\s+|\s+$/g, '') !== undefined){
                if(tempMap.get(k.getDepth()) === undefined){
                    tempMap.set(k.getDepth(), [k.getText()]);    
                }else{
                    tempMap.set(k.getDepth(), tempMap.get(k.getDepth()).concat([k.getText()]));
                }
            }else{
