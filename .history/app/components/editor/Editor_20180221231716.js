...
class Editor2 extends Component {
    ...
    saveDepthMap(){
        ...
        contentblockMap.forEach((k,v) => blockMap.set(k.getText(), k.getDepth()));
        ...
        contentblockMap.forEach(k => {
            ...
            if(text.replace(/^\s+|\s+$/g, '') !== undefined && text !== ''){
                if(!depthMap.has(depth)){
                    ...
                    childMap = this.saveChildMap(childMap, depthMap, depth, text, contentblockMap);
                    this.setState({
                        depthMap,
                        childMap
                    }, () => {
    ...
    saveChildMap(childMap, depthMap, depth, text, blockMap){
        if(depthMap.has(depth-1)){
            ...
            let stack = this.createStack(new Stack(), newKey, blockmap, depthMap);
            while(!stack.isEmpty()){
