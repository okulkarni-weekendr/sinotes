...
class Editor2 extends Component {
    ...
    addToDepthMap(){
        ...
        let depth = editorState.getCurrentContent().getBlockMap().getBlockForKey(editorState.getSelection().getStartKey()).getDepth();
        let text = editorState.getCurrentContent().getBlockMap().getBlockForKey(editorState.getSelection().getStartKey()).getText();
    ...
    addToChildMap(depth, text){
        ...
        if(depthMap.has(depth-1)){
            ...
            console.log(key, tempArray, 'depth and text of currently inserting text ->', depth, text);
