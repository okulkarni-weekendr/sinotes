    ...
    saveDepthMap(){
        ...
        blockMap.forEach(k => {
            ...
            if(value.replace(/^\s+|\s+$/g, '') !== undefined){
                if(!depthMap.has(key)){
                    this.setState({
                        depthMap: depthMap.set(key, [value])
                    }, () => {
                        console.log('depthMap when no key -->', key, value);
                        this.saveChildMap(key, value);
                    });
                }else{
                    this.setState({
                        depthMap: depthMap.set(key, depthMap.get(key).concat([value]))
                    }, () => {
                        console.log('depthMap has key -->', key, value);
                        this.saveChildMap(key, value);
                    });
                }
            }
