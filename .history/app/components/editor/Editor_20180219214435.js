...
class Editor2 extends Component {
    ...
    handleKeyCommand(command){
        ...
        if (command === 'saveChart') {
            ...
            childMap.forEach((k,v) => console.log(k,v));
            return 'handled';
        }
        return 'not-handled';
    }
    ...
    saveChildMap(key, value){
        ...
        if(depthMap.get(key - 1) !== undefined){
            let newKey = depthMap.get(key - 1)[depthMap.get(key - 1).length - 1];
            this.setState({
                childMap: childMap.set(newKey, childMap.get(newKey).concat([key])).set(value, [])
            });
        }else{
            this.setState({
