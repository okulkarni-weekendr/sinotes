...
class Editor2 extends Component {
    ...
    addToDepthMap(){
        ...
        let depth = this.state.editorState.getCurrentContent().getBlockForKey(this.state.editorState.getSelection().getStartKey()).getDepth();
        let text = this.state.editorState.getCurrentContent().getBlockForKey(this.state.editorState.getSelection().getStartKey()).getText();
    ...
    addToChildMap(depth, text){
        ...
        if(depthMap.has(depth-1)){
            ...
            console.log(newChildMap);
            console.log(depthMap);
            let array = newChildMap.get(key).concat([text]);
            this.setState({
                childMap:  newChildMap.set(key, array)
            });
        }else{
            console.log('when no depth exists ->', text);
            let array = [];
            this.setState({
    ...
    handleKeyCommand(command){
        ...
        if (command === 'saveChart') {
            this.addToDepthMap(this.state.editorState, this.state.depthMap);
            return 'handled';
